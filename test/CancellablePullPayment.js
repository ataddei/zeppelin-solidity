...
contract('CancellablePullPayment', function(accounts) {
  ...
  it("can't call asyncSend externally", async function() {
    let ppc = await CancellablePullPaymentMock.new();
    assert.isUndefined(ppc.asyncSend);
  });
    
  it("can record an async payment correctly", async function() {
    let AMOUNT = 100;
    let ppce = await CancellablePullPaymentMock.new();
  ...
  it("can withdraw payment", async function() {
    ...
    let ppce = await CancellablePullPaymentMock.new({value: AMOUNT});
    ...
    it("it can reject payment", async function() {
        let AMOUNT = 17*1e18;
        let payee = accounts[1];
        let initialBalance = web3.eth.getBalance(payee);
        ...
        let reject = await ppce.rejectPayments();
        let withdraw = await ppce.withdrawPayments({from: payee});
        let payment2 = await ppce.payments(payee);
        assert.equal(payment2, 0);
        ...
        let balance = await web3.eth.getBalance(payee);
